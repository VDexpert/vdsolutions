...
urlpatterns = [
    ...
    path('some-error/', TemplateView.as_view(template_name='users/service/some_error.html'), name='some_error'),
    path('update-product-range/<int:pk>/', UpdateRangeProductUpdateView.as_view(), name='upd_prod_range'),
    path('success-updating-range/', TemplateView.as_view(template_name='users/service/after_success_update_product_range.html'),
         name='success_upd_prod_range'),
    path('success-ordering-trials/', TemplateView.as_view(template_name='users/service/after_ordering_trials_update_range_product.html'),
         name='after_ordering_trials'),
    path('ordering-trials/', OrderTrialUpdateRangeProductTemplateView.as_view(),name='order_trials'),
    path('error-send-ban-status/', TemplateView.as_view(template_name='users/service/send_ban_status_error.html'), name='error_send_ban_status'),
