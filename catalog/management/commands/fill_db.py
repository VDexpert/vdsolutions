...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        categories = [
            {'category_name': 'Автоматизация', 'description_for_products': 'Полное описание категории для каталога Программы для автоматизации различных бизнес-процессов',
             'annotation_products': 'Аннотация для каталога Программы для автоматизации различных бизнес-процессов',
             'annotation_posts': 'Аннотация для блога Программы для автоматизации различных бизнес-процессов', 'empty': 'false',
             'description_for_posts': 'Полное описание категории для блога Программы для автоматизации различных бизнес-процессов'},

            {'category_name': 'Парсинг', 'description_for_products': 'Полное описание категории для каталога Программы для парсинга больших баз данных',
             'annotation_products': 'Аннотация для каталога Программы для парсинга больших баз данных',
             'annotation_posts': 'Аннотация для блога Программы для парсинга больших баз данных', 'empty': 'false',
             'description_for_posts': 'Полное описание категории для блога Программы для парсинга больших баз данных'},

            {'category_name': 'Нейронные сети', 'description_for_products': 'Полное описание категории для каталога Программы, использующие в своей работе технологии искусственного интеллекте',
             'annotation_products': 'Аннотация для каталога Программы, использующие в своей работе технологии искусственного интеллекте',
             'annotation_posts': 'Аннотация для блога Программы, использующие в своей работе технологии искусственного интеллекте', 'empty': 'false',
             'description_for_posts': 'Полное описание категории для блога Программы, использующие в своей работе технологии искусственного интеллекте'}
        ]
        ...
        posts = [
            ...
            {'title': 'Электронный документооборот (ЭДО): какие бывают виды, как работает, особенности, функции', 'user': User.objects.all().get(id=1), 'category': Category.objects.all().get(id=2),
             'slug': translit.do('Электронный документооборот (ЭДО): какие бывают виды, как работает, особенности, функции'), 'content': plugs.text()},
            {'title': 'CRM-системы для малого бизнеса: какую выбрать, обзор лучших', 'user': User.objects.all().get(id=1), 'category': Category.objects.all().get(id=2),
            ...
            {'title': 'CRM-системы для агентства недвижимости: лучшие программы для риэлторов', 'user': User.objects.all().get(id=2), 'category': Category.objects.all().get(id=3),
             'slug': translit.do('CRM-системы для агентства недвижимости: лучшие программы для риэлторов'), 'content': plugs.text()},
            {'title': 'Система электронного документооборота EDI: платформа для обмена данными', 'user': User.objects.all().get(id=2), 'category': Category.objects.all().get(id=3),
             'slug': translit.do('Система электронного документооборота EDI: платформа для обмена данными'), 'content': plugs.text()},
            {'title': 'Бухгалтерские программы: список программного обеспечения для ведения бухгалтерии', 'user': User.objects.all().get(id=2), 'category': Category.objects.all().get(id=3),
